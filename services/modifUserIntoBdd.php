...
if (
    ...
    if (empty($_POST["user_role"]) || empty($_POST["user_pseudo"]) || empty($_POST["user_email"]) || empty($_POST["user_birthday"])) {
        ...
        try {
            ...
            if (empty($pswrdHash)) {
                ...
                $request = $db->prepare("UPDATE users SET user_role = ?,user_firstname = ?,user_lastname = ?,user_pseudo = ?,user_email = ?, user_birthday = ? WHERE user_id = ?");

                //Envoi
                $request->execute([$role, $firstname, $lastname, $pseudo, $email, $birthday, $id]);
